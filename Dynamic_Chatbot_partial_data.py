...
def generate_loss(model, tokenizer, interlocuter, bert_model, bert_tokenizer, persona_selector, persona_pool, arg, input_ids, personalities):
    # history = []
    model.eval()
    # for param in persona_selector.id_selector.named_parameters():
    #     print(param)
    interlocuter.eval()
    ...
    for i in range(len(history_enc)):
        ...
        # print("history_enc[i][0] id \n", tokenizer.decode(history_enc[i][0]))
        # print("inter_persona[i] is \n", tokenizer.decode(inter_persona[i]))
    # exit()
    # index = 0
    for i in range(len(history_enc)):
        # history_enc[i] is [s0]
    ...
    selected_persona, log_prob, entropy = select_persona(persona_selector, persona_pool, persona_history, bert_tokenizer, bert_model)
    # print("selected persona is \n", selected_persona)
    ...
    for i in range(len(personalities)):
        ...
        # print("s2 is \n", tokenizer.decode(s2, skip_special_tokens=True))
    ...
    selected_persona_2, log_prob, entropy = select_persona(persona_selector, persona_pool, persona_history_2, bert_tokenizer, bert_model)
